...
**Phase 2: sidecar-native (editor DOM never contains floating markers)** (2026-07-22)

Editor content **truly separated** from floating overlay layer. Floating
elements di JS state array (`floatingElements`), editor DOM completely
bebas dari floating markers. Insert flow → sidebar state langsung, tidak
lewat `editor.insertContent()`. Sidebar "Floating Elements" panel show
list dgn per-element cards untuk edit position/z-index/delete.

**Industry precedent (docstring citations)**:
- **Google Docs** — `<a:drawing>` XML nodes separate dari `<w:t>` text runs
- **MS Word OOXML** — `<w:drawing>` element di WordprocessingML terpisah dari text
- **Figma / Sketch / Adobe InDesign** — Layer panel model, tiap object standalone
- **CKEditor 5 Widgets** — atomic non-editable widget units dgn sidebar
- **Slate.js `void` nodes** — non-editable JSON structures separate dari text
- **Prosemirror NodeView** — custom view for embedded content
- **Notion Callout / Embed** — sidebar/toolbar untuk non-inline content

**Changed — designer.php load flow**

- Read `floating_elements` JSON column → passed ke JS as
 `window.__EZDOC_FLOATING_INIT` (via inline `<script>` before include partial)
- Editor template_html cleaned dgn `FloatingExtractor::extract()` — auto-strip
 markers dari HTML (defensive: catches legacy rows dgn markers still in HTML)
- Legacy row (JSON NULL): auto-migrate on load — extract dari HTML markers,
 populate JSON path, strip markers dari editor content
- Removed `FloatingInjector::inject()` call — no longer inject markers ke editor

**Added — sidebar panel**

- New collapsible panel "Floating Elements" di designer sidebar
 (position between Logo Panel dan TTD Panel)
- Live-rendered via `renderFloatingList()` — per-element card dgn:
 - Bootstrap icon per type (image/qr-code/pen/stamp)
 - Element id + type badge
 - Position X + Y inputs (px, editable, live-update state)
 - Z-index select (front / behind)
 - Delete button (× red icon)
- Empty state: hint "Insert floating logo/TTD/QR/materai from toolbar → appears here (not in editor)"
- Counter badge di panel header (number of floating elements)

**Changed — insert functions** (`insertLogoPrompt`, `insertQrPrompt`,
`insertTtdPrompt`, `insertMateraiPrompt`)

- Untuk `mode === 'front' || mode === 'behind'` (floating variants):
 - Skip `editor.insertContent()` entirely
 - Call `addFloatingElement()` — push spec ke state array
 - Sidebar re-render + `markDirty()` triggered
 - Default positions: logo/qr 20,20 · ttd 50,100 · materai 350,500
- Inline mode (`mode === 'inline'`) unchanged — stays in editor content
 (semantically correct for text-flow elements)

**Added — state mutation helpers**

- `addFloatingElement(spec)` — push new element (id/type/x/y/z/width/data),
 trigger render + dirty
- `removeFloatingElement(idx)` — remove by index
- `updateFloatingPosition(idx, axis, value)` — mutate X or Y from input event
- `updateFloatingZIndex(idx, value)` — front/behind switch
- `escapeHtml(s)` — XSS-safe attr injection helper

**Changed — save flow**

- `formData.append('floating_elements_json', JSON.stringify(floatingElements))`
- Backend already accepts this field (v0.9.12 Phase 1 dual-write flow),
 no server change needed
- Save extractor fallback (extract-from-HTML) becomes redundant tapi
 preserved untuk backward-compat kalau consumer bypass sidebar UI

**Zero UX regression**

- Existing templates (JSON populated OR legacy markers in HTML) load OK
- Save round-trip preserves position/type/id/z-index data
- Empty templates (create mode) start with empty `floatingElements = []`
